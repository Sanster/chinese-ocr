...
def model(img, model='keras', adjust=False, detectAngle=False):
    ...
    print("ctpn time: {:.03f}s".format(time.time() - ctpn_start_time))

    sort_box_start_time = time.time()
    text_recs = sort_box(text_recs)
    print("sort_box time: {:.03f}s".format(time.time() - sort_box_start_time))

    crnn_start_time = time.time()
    result = crnnRec(img, text_recs, model, adjust=adjust)
    print("crnn time: {:.03f}s".format(time.time() - crnn_start_time))
